...
public class HospitalTableMainWindow extends JPanel {
    ...
    public void showDatabasePart(int startIndex) {
        if (model != null) {
            ...
            view.setTableData(databasePart);
            updateDatabaseInformationLabels();
        } else
            JOptionPane.showMessageDialog(new JFrame(), "Database not connected");
    }
    ...
    private class HospitalTableController extends AbstractHospitalTableController {
        ...
        public void actionPerformed(ActionEvent mouseEvent) {
            if (mouseEvent.getSource() == view.getBackButton()) {
                if (pageNumber > 0) pageNumber--;
            } else if (mouseEvent.getSource() == view.getForwardButton()) {
                if (pageNumber < pagesCount) pageNumber++;
            } else if (mouseEvent.getSource() == view.getStartPageButton()) {
                pageNumber = 0;
            } else if (mouseEvent.getSource() == view.getEndPageButton()) {
                pageNumber = pagesCount;
            } else if (mouseEvent.getSource() == view.getRowsCountAtThePageTextField()) {
                setRowsAtPageCount();
            }
        ...
        private void setRowsAtPageCount() {
            int result = 1;
            try {
                result = Integer.parseInt(view.getRowsCountAtThePageTextField().getText());
                view.getRowsCountAtThePageTextField().setBackground(Color.white);
            } catch (NumberFormatException e) {
                view.getRowsCountAtThePageTextField().setBackground(Color.red);
            }
            if (result > 0 && result < 15) {
                rowsAtPage = result;
                pagesCount = (int) Math.ceil(((double) model.getDatabaseSize()) / rowsAtPage) - 1;
                pageNumber = 0;
                view.resizeTable(rowsAtPage, HospitalTable.COLUMN_NAMES.length);
            } else
                view.getRowsCountAtThePageTextField().setBackground(Color.red);
        }
    }
